...
%-------------------------NORMALIZATION
Mat_train_tensor_n(:,1:pos_label-1,:) = normalize(Mat_train_tensor(:,1:pos_label-1,:), 'range', [-1 1]);
Mat_test_tensor_n(:,1:pos_label-1,:) = normalize(Mat_test_tensor(:,1:pos_label-1,:), 'range', [-1 1]);
Mat_train_tensor_n(:,pos_label,:) = Mat_train_tensor(:,pos_label,:);
Mat_test_tensor_n(:,pos_label,:) = Mat_test_tensor(:,pos_label,:);

%-------------------------CLASSIFICATORE_RVFL
for kk = 1:N_fold
        % azzero le variabili seguenti:
        H=zeros(N_train,N_kernel*N_features); % hidden matrix usata nel training
        H_test=zeros(N_test,N_kernel*N_features); % hidden matrix usata nel test

        for i=1:N_train % righe (training set)
                % distanza euclidea normalizzata per la funzione "h":
                H(i,:) = Cfl(Mat_train_tensor_n(i,1:pos_label-1,kk),N_kernel);
                %Hq(i,jj) = apx_normal_dist(Mat_train_tensor(i,1:pos_label-1,kk)*128,mu_q(jj,:)-mu_zp,sigma_q(jj,:)-sigma_zp);
        end
        %Hq = double(Hq);
        I = eye(N_kernel*N_features); % matrice identità quadrata
        ...
        for i=1:N_test
                H_test(i,:) = Cfl(Mat_test_tensor_n(i,1:pos_label-1,kk),N_kernel);
                %Hq_test(i,jj) = apx_normal_dist(Mat_test_tensor(i,1:pos_label-1,kk)*128,mu_q(jj,:)-mu_zp,sigma_q(jj,:)-sigma_zp); 
        end
        ...
        c = Mat_test_tensor_n(:,pos_label,kk) == y_pred(:,kk);
        f = sum(c);
